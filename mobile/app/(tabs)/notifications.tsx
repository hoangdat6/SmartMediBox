...
export default function NotificationsScreen() {
  ...
  const { notifications, loading, error, fetchNotifications, clearAllNotifications } = useNotificationsStore();
  ...
  const renderNotificationItem = ({ item }: { item: NotificationWithId }) => {
    ...
    return (
      <TouchableOpacity style={[styles.notificationItem, { backgroundColor: colors.card }]}>
        ...
        <View style={styles.notificationContent}>
          ...
          <Text style={[styles.notificationMessage, { color: theme === 'dark' ? '#AAA' : '#555' }]}>
            {item.message}
          </Text>
        </View>
      </TouchableOpacity>
  ...
  const EmptyNotifications = () => (
    <View style={styles.emptyContainer}>
      <MaterialCommunityIcons name="bell-off" size={50} color={theme === 'dark' ? '#555' : '#ccc'} />
      <Text style={[styles.emptyText, { color: theme === 'dark' ? '#777' : '#999' }]}>Chưa có thông báo</Text>
    </View>
  );
  ...
  const FooterComponent = () => {
    ...
    return (
      <TouchableOpacity 
        style={[styles.clearAllButton, { backgroundColor: theme === 'dark' ? '#333' : '#f0f0f0' }]}
        onPress={handleClearAll}
      >
        <Text style={[styles.clearAllText, { color: theme === 'dark' ? '#CCC' : '#666' }]}>Xóa tất cả</Text>
      </TouchableOpacity>
    );
  };
